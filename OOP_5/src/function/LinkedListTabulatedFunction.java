...
public class LinkedListTabulatedFunction implements TabulatedFunctionImpl, Serializable, Cloneable {
    ...
    public boolean equals(Object o) {
        ...
        if (!(o instanceof TabulatedFunctionImpl))  //Если не реализует интерфейс - то false
            return false;
        ...
        if (o instanceof LinkedListTabulatedFunction) {  //Если приводится к классу, то происходит проверка
            ...
            if (((TabulatedFunctionImpl) o).getPointsCount() != this.countPoints)
                return false;

            FunctionNode thisNode = this.head.next;
            for (int i = 0; i < this.countPoints; ++i) {
                if (!((TabulatedFunctionImpl) o).getPoint(i).equals(thisNode.fp)) return false;
